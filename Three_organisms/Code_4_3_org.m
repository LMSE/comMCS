     ...
     if size(rxnKOs,1) < 30
        ...
        for m = 1:size(rxnKOs,1)
            ...
            elseif contains(Com.rxns(rxnKOs(m)),"_org2")
                organism(m) = 2;
            elseif contains(Com.rxns(rxnKOs(m)),"_org3")
                organism(m) = 3;
            else
                organism(m) = 0;
            end
   ...
   rmvRxns(1,1:size(kill_genes,1)+1) = 0;
        for j=1:length(Com.grRules)
            ...
            elseif contains(Com.rxns(j),"_org2")
                type = 2;
            elseif contains(Com.rxns(j),"_org3")
                type = 3;
            else
                type = 0;               
            end
     ...
     if sum(works) > 0
        real_removal(1:size(find(ismember(rmvRxns(:,j),1)),1),i) = Com.rxns(find(ismember(rmvRxns(:,j),1)));
     end
     end
  end
%   save ('Gene_mcs_3_org.mat','real_removal');
%   save ('MCS_valid_3_org.mat','MCS_valid');