...
load iAF_and_iYO_MCS  % your MCS file
...
Com.lb(idx.glc) = 0;
Com_save = Com;
mcs(end+1:size(Com.rxns,1),:) = zeros(size(Com.rxns,1)-size(mcs,1),size(mcs,2));
...
% removed
 ...
 % Find rxns that will be ko'ed with each possible solutions
  ...
  save ('Gene_mcs_iAF_iYO.mat','real_removal');
  save ('MCS_valid_iAF_iYO.mat','MCS_valid');
  save ('mcs_val_iAF_iYO.mat','mcs_val');